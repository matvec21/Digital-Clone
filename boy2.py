...
def set_lora_scale(model, adapter_name, scale):
    for module in model.modules():
        if isinstance(module, LoraLayer):
            module.set_scale(adapter_name, scale)

set_lora_scale(model, 'default', 0.7)

# %%
BRAIN_PROMPT = {
    'role': 'system', 
    'content': (
        'Твоя задача: составить краткий ПЛАН ответа. '
        'Теги [MEDIA: description] означают изображение, которые ты видишь. '
        'ПАМЯТЬ: Учитывай важные данные из прошлых сообщений (пароли, имена, договоренности). '
        'ЗАПРЕЩЕНО: Использовать вежливость, давать советы или писать готовое сообщение. Только логический скелет.'
    )
}
VOICE_PROMPT = {
    'role': 'system', 
    'content': (
        'Ты — цифровой клон. Отвечай на сообщения пользователя, используя факты из блока [ПЛАН] и историю сообщений. '
        'Теги [MEDIA: description] означают изображение, которые ты видишь. '
        'Пиши как живой человек, используй свой сленг.'
    )
}
...
def generate_answer(history):
    prompt_brain = tokenizer.apply_chat_template([BRAIN_PROMPT] + history, tokenize = False, add_generation_prompt = True)
    inputs_brain = tokenizer(prompt_brain, return_tensors = 'pt').to(model.device)

    with torch.no_grad(), model.disable_adapter():
        brain_outputs = model.generate(
            **inputs_brain,
            max_new_tokens = 30,
            temperature = 0.1,
    ...
    plan_tokens = brain_outputs[0][inputs_brain.input_ids.shape[-1]:]
    plan_text = tokenizer.decode(plan_tokens, skip_special_tokens=True).strip()

    history = deepcopy(history)
    history = [VOICE_PROMPT] + history
    history[-1]['content'] += f"\n\n[ПЛАН: {plan_text}]"

    print('ПЛАН ОТВЕТА:', repr(plan_text))

    prompt_voice = tokenizer.apply_chat_template(history, tokenize = False, add_generation_prompt = True)
    inputs_voice = tokenizer(prompt_voice, return_tensors = 'pt').to(model.device)
...
#           sequence_bias = {tuple([new_token_id]): 1.5, tuple([skobka]): -0.5, tuple([eos_token_id]): -0.5, tuple([square]): 4.0},
            sequence_bias = {tuple([square]): 2.0},
    ...
    output_tokens = outputs[0][inputs_voice.input_ids.shape[-1]:]
...
def me_answer(message : types.Message, text : str):
    ...
    try:
        ...
        if len(chat_history) > MAX_HISTORY:
            user_memories[message.chat.id] = chat_history[-MAX_HISTORY:]
    finally:
        stop_typing.set()
        typing_thread.join()
...
def ontext(message : types.Message):
    ...
    if text.lower() == 'clear':
        user_memories[message.chat.id] = list()
        bot.send_message(message.chat.id, text = 'Прошли сутки...')
        return

    me_answer(message, text)
...
def on_photo(message: types.Message):
    ...
    try:
        ...
        me_answer(message, text_with_photo)
